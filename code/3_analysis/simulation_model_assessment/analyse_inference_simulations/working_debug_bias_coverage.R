...
model <- "diagREDM_" ## estimate of lambda needs to be exponentiated and multiplied by 1000
model <- "fullREDM_"

idx_dataset_betaintercept <- "betainterceptPCAWG2"
idx_dataset_betaslope <- "betaslopePCAWG2"
idx_dataset_cov <- "covmatPCAWG2"
arg_n <- 200
arg_nlambda <- 180
arg_lambda <- 100
arg_d <- 6
...
ggplot((summaries_melt),
       aes(x=true, y=value))+geom_point()+
