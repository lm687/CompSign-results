...
if(local){
  ...
  lst <- list.files(paste0("../data/assessing_models_simulation/inference_results/TMB/", optimiser, "/", name_dataset, arg_n, '_',
                           arg_nlambda, '_', arg_lambda, '_', arg_d, '_', arg_beta_gamma_shape, '_', model, '_', idx_dataset_betaintercept, '_',
                           idx_dataset_betaslope, '_', idx_dataset_cov, '/'), full.names = T)
  ...
  # lst <- lst[grepl('200_180_100_5_0', lst)]
  lst <- lst[grepl(paste0(arg_n, '_', arg_nlambda, '_', arg_lambda, '_', arg_d, '_', arg_beta_gamma_shape), lst)]
  ...
  opt$n = arg_n
  opt$nlambda = arg_nlambda
  opt$lambda = arg_lambda
  opt$d = arg_d
  opt$beta_gamma_shape <- arg_beta_gamma_shape
  ...
  option_list = list(
    ...
    make_option(c("--d"), type="character", default=NA,
                help="Number of features (character or numeric)", metavar="character"),
    make_option(c("--n"), type="character", default=NA,
                help="Number of samples (character or numeric)", metavar="character"),
    make_option(c("--nlambda"), type="character", default=NA,
                help="Parameter lambda for Poisson draws of number of mutations in sample (character or numeric)", metavar="character"),
    ...
    make_option(c("--lambda"), type="character", default=0,
                help="Overdispersion parameter", metavar="character")
  );
...
first_part_output <- paste0("../results/results_TMB/simulated_datasets/bias_and_coverage/",
                            # "multiple_", 
...
if(opt$dataset_generation %in% c("GenerationJnormMax", "GenerationJnormInv")){
  ...
  cat('Creating ', paste0(first_part_output, "bias.pdf"), '\n')
  ...
  cat('Creating ', paste0(first_part_output, "bias_betas.pdf"), '\n')
  ggplot(cbind.data.frame(summaries_melt[summaries_melt$Var1 == 'beta',],  intslope=c('Intercept', 'Slope')),
  ...
  cat('Creating ', paste0(first_part_output, "bias_betas_v2_vert.pdf"), '\n')
  ggplot(cbind.data.frame(summaries_melt[summaries_melt$Var1 == 'beta',],  intslope=c('Intercept', 'Slope')),
         aes(x=(1+idx_param) %/% 2, y=subtract, group=idx_param))+
    geom_abline(slope = 0, intercept = 0, lty='dashed', col='blue')+
    geom_boxplot()+#+facet_wrap(~Var1, scales = "free", nrow=1)+
    ...
    theme_bw()+
    facet_wrap(.~intslope, drop = T, labeller = as_labeller(appender, 
                                                            default = label_parsed), ncol = 1)+
    theme(axis.text.x=element_blank(),
          axis.ticks.x=element_blank())+theme(legend.position = "bottom")#+scale_color_manual(values = c('#c2b2b2', '#72081d'))
  ggsave(paste0(first_part_output, "bias_betas_v2_vert.pdf"), width = 2, height = 3)
  ...
  cat('Creating ', paste0(first_part_output, "coverage.pdf"), '\n')
  ggplot(df_coverage,
  ...
  cat('Creating ', paste0(first_part_output, "coverage_beta.pdf"), '\n')
  ggplot(df_coverage[df_coverage$type_param %in% c("Intercept",  "Slope"),],
         aes(x=parameter, y=CI, group=1, shape=type_param))+
  ...
  if((length(runs_nonconverged_list) > 0) & (length(runs) > 0) ){
    ...
    cat('Creating ', paste0(first_part_output, "comparison_nonconv.pdf"), '\n')
