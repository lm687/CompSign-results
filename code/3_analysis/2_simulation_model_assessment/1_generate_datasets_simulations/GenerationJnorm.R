...
if(debug){
  setwd("code/")
  opt <- list();
  opt$datasetgeneration='GenerationJnorm'; opt$model='fullREDM'; opt$fixed_beta_intercept = 'betaintercept1d4';
  opt$fixed_beta_slope = 'betaslope1d4'; opt$sdRE_input = 'covmat1d4';  opt$n = 200; opt$nlambda = 180; opt$lmbda = 2; opt$d = 5
}
...
for(opt_param in c('d', 'lambda', 'n', 'nlambda')){
  ...
  if(is.na(opt_param_as_numeric)){
    ...
    if(opt_param == 'lambda'){
      ...
      lambda = rep(as.numeric(opt$lambda), 2) ## overdispersion scalars. Lower value -> higher overdispersion
    }else{
      ...
      if(opt_param == 'nlambda'){
        Nm_lambda = as.numeric(opt$nlambda) ## lambda parameter for number of mutations per sample (i.e. a sample in a group)
      }
...
if(is.null(opt$beta_slope_input)){
  ...
  if(is.na(opt$beta_slope_input)){
    ...
    stopifnot( (d-1) == length(read_beta_slope))
...
## Create the counts
if(is.list(Nm_lambda)){
  Nm = c(sample(x = Nm_lambda[[1]], size = n, replace = T), ## group 1
         sample(x = Nm_lambda[[2]], size = n, replace = T)) ## group 2
}else{
  cat('Nm_lambda: ')
  print(Nm_lambda)
