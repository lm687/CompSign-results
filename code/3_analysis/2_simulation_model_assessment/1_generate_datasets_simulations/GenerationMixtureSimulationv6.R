...
## Same as v5, but this time the signatures are chosen at random from each group, and not per patient (so the intercepts in RE should not matter)
...
ct1 <- load_PCAWG("Prost-Adenoca", typedata = "signaturesPCAWG", simulation = F, path_to_data="../data/", override_warning_X_Z=T)
## selecting only the top 4 signature exposures
selected_sigs <- names(sort(colSums(ct1$Y), decreasing = T)[1:4])
ct1 <- give_subset_sigs_TMBobj(ct1, sigs_to_remove = colnames(ct1$Y)[!(colnames(ct1$Y) %in% selected_sigs)])
...
if((beta_gamma_shape == -999)){
  ...
  props_mix <- softmax(c(0, beta_gamma_shape))
}
print(props_mix)
...
if(nrow(ct1) < (2*n)){
  ...
  sample_ids2 <- sample(1:(nrow(ct1)), size = n, replace = T)
}else{
  sample_ids <- sample(1:(nrow(ct1)), size = n)
  sample_ids2 <- sample(1:(nrow(ct1)), size = n)
}

## number of mutations in each patient-specific subsample - not related to group
nlambda_vec <- sample(c(rowSums(ct1), rowSums(ct2)), size = length(sample_ids)*2)

## the number of mutations is drawn at random from the observed data (but randomising the two groups)

W <- rbind(t(sapply(1:n, function(id_patient){
  rmultinom(1, nlambda_vec[id_patient], normalise_rw(ct1[sample_ids[id_patient],]))
})),
t(sapply(c(1:n), function(id_patient){
  rmultinom(1, round(nlambda_vec[id_patient+length(sample_ids)]*props_mix[1]), 
            normalise_rw(ct1[sample_ids[id_patient],]))+
    rmultinom(1, round(nlambda_vec[id_patient+length(sample_ids)]*props_mix[2]), 
              normalise_rw(ct2[sample_ids2[id_patient],]))
})))
