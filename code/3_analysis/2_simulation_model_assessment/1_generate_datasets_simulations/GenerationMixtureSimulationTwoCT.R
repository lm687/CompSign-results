...
## Using data from two cancer types
...
cat('Getting signature definitions\n')
...
cat('Loading cancer datasets\n')

ct1 <- load_PCAWG("Lymph-CLL", typedata = "signaturesPCAWG", simulation = F, path_to_data="../data/", override_warning_X_Z=T)
## subset of sigs for Breast-AdenoCa
ct2 <- load_PCAWG("Breast-AdenoCa", typedata = "signaturesPCAWG", simulation = F, path_to_data="../data/", override_warning_X_Z=T)

## median(c(rowSums(ct1$Y), rowSums(ct2$Y))) #1773.5
selected_sigs <- names(sort(colSums(ct2$Y), decreasing = T)[1:4])
ct2 <- give_subset_sigs_TMBobj(ct2, sigs_to_remove = colnames(ct2$Y)[!(colnames(ct2$Y) %in% selected_sigs)])

ct1 <- ct1$Y
ct2 <- ct2$Y


all_sigs <- unique(c(colnames(ct1), colnames(ct2)))

## unify
ct1 <- ct1[,match(all_sigs, colnames(ct1))]
ct2 <- ct2[,match(all_sigs, colnames(ct2))]
ct1[is.na(ct1)] <- 0
ct2[is.na(ct2)] <- 0
stopifnot(ncol(ct1) == ncol(ct2))
colnames(ct2) <- colnames(ct1) <- all_sigs
...
cat('Sampling samples\n')

if(nrow(ct1) < (n)){
  warning('Cancer type has fewer observations than runs. Sampling with replacement\n')
  ## signature exposures are chosen for each subsample by sampling from the group, not pairing them
...
if(nrow(ct2) < (n)){
  warning('Cancer type has fewer observations than runs. Sampling with replacement\n')
  ## signature exposures are chosen for each subsample by sampling from the group, not pairing them
  sample_ids2 <- sample(1:(nrow(ct2)), size = n, replace = T)
}else{
  sample_ids2 <- sample(1:(nrow(ct2)), size = n)
}
## the number of mutations is fixed, as specified in the argument

cat('Creating count exposures\n')
W <- rbind(t(sapply(1:n, function(id_patient){
  rmultinom(1, opt$nlambda, normalise_rw(ct1[sample_ids[id_patient],]))
})),
t(sapply(c(1:n), function(id_patient){
  rmultinom(1, round(opt$nlambda*props_mix1[1]), normalise_rw(ct1[sample_ids[id_patient],]))+
    rmultinom(1, round(opt$nlambda*props_mix1[2]), normalise_rw(ct2[sample_ids2[id_patient],]))
})))
colnames(W) <- colnames(ct1)

## These are the exposures. From these exposures, get mutations.

cat('Simulating from signatures\n')
...
W_muts <- simulate_mutations_from_signatures(W)
