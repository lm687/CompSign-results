## Same as GenerationMixturewithCCFa, but with another cancer type. Signature extraction with mutSigExtractor
...
## note that these are mutations exactly as found in the PCAWG vcfs
...
mutations_per_group = sapply(samples, function(it_sample){
  sapply(1:length(mutations_per_group[[it_sample]]), function(group){
    if(group == 1){
      ...
      muts <- sample(c(muts1, muts2), replace = F, size = nrow(mutations_per_group[[it_sample]][[group]])) ## shuffle mutations that come from one group or the other
    }else{
      stop
    }
...
W_bins_QP_list <- extract_sigs_TMB_obj(dataset_obj_trinucleotide = objects_counts_in_list_bins,
                                       ...
                                       signature_definition = sigs_cosmic0, signature_fitting_method = 'mutSigExtractor')
