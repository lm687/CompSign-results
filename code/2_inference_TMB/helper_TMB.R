...
L_to_cov_wrapper_TMB_v2 <- function(TMB_obj, name_cov_par='cov_RE', d=NULL){
  .x <- summary(TMB_obj)
  if(is.null(d)){
...
wrapper_run_TMB = function(model, object=NULL, smart_init_vals=T, use_nlminb=F, initial_params=NULL, return_opt=F, generalised_X=F){
  ## sort_columns=F, 
  ...
  if(is.null(data$z)){
    warning('object$z does not exist - n is taken to be the number of rows of object$x')
   n <- nrow(data$x) 
  }else{
    n <- ncol(data$z) ## number of INDIVIDUALS, not samples
  }
  ...
  if(smart_init_vals){
    ...
    if(!generalised_X){
      ...
      .x_multinom = multinom(data$Y ~ factor(null_to_0(apply(data$x[,-1], 1, function(i) which(i == 1)))))
      # length(unique(factor(apply(data$x[,-1], 1, function(i) which(i == 1)))))
      # .x_multinom = multinom(data$Y ~ cbind.data.frame(intercept=1, chrom=factor(apply(data$x[,-1], 1, function(i) which(i == 1)))))
  ...
  if(model == "fullRE_M"){
    ...
    rdm_vec <- "u_large"
  }else if(model == "diagRE_DMreducedmodel"){
    data$num_individuals = n
    data$lambda_accessory_mat = (cbind(c(rep(1,n),rep(0,n)), c(rep(0,n),rep(1,n))))
    data$x =   matrix(data$x[,1], ncol=1) ### difference to full model
    parameters$beta = matrix(parameters$beta[1,], nrow=1) ### difference to full model
    parameters$log_lambda = matrix(c(2,2))
    parameters$cov_par_RE = NULL
    dll_name <- "diagRE_ME_dirichletmultinomial"
    rdm_vec <- "u_large"
    ...
    rdm_vec <- c("u_large1", "u_large2","u_large3", "u_large4")
  }else if(model == "diagREcor_DM"){
    ## logs_sd_RE is now bounded
    data$num_individuals = n
    data$lambda_accessory_mat = (cbind(c(rep(1,n),rep(0,n)), c(rep(0,n),rep(1,n))))
    parameters$log_lambda = matrix(c(2,2))
    parameters$cov_par_RE = NULL
    dll_name <- "diagREcor_ME_dirichletmultinomial"
    rdm_vec <- "u_large"
    
    stopifnot(use_nlminb) ## in this script bounds can only be specified using nlminb
    uppers <- rep(Inf, length(unlist(parameters)))
    uppers[grepl('logs_sd', uppers)] <- 1
    lowers <- rep(-Inf, length(unlist(parameters)))
    lowers[grepl('logs_sd', lowers)] <- 0
  ...
  if(use_nlminb){
    if(override_opt){
      ## use previously-defined opt
      cat('Using previously-defined opt parameters (bounds)\n')
...
plot_betas_generalised <- function(TMB_obj, return_plot=F, return_ggplot=T, p, names_covariates=NULL,
                                   annotate_pval=T, interchange_facet_x=F, angle_arg=0){
  cat('Using the generalised version of <plot_betas>, for p!=2\n')
  if(is.null(names_covariates)){
    names_covar = c('Intercept', paste0('Slope ', 1:(p-1)))
  }else{
    names_covar = names_covariates
  }
  pval_res <- sapply(1:p, function(p_it){
    bool_which_p = rep(FALSE, p)
    bool_which_p[p_it] = TRUE
    
    wald_generalised(v = python_like_select_rownames(summary(TMB_obj), 'beta')[bool_which_p,1],
                     sigma = TMB_obj$cov.fixed[which(rownames(TMB_obj$cov.fixed) == 'beta')[bool_which_p],
                                               which(rownames(TMB_obj$cov.fixed) == 'beta')[bool_which_p]])
    
  })
  .summary_betas <- summary(TMB_obj)
  .summary_betas <- cbind.data.frame(python_like_select_rownames(.summary_betas, 'beta'),
                                     type_beta=rep(names_covar),
                                     LogR=rep(1:(nrow(python_like_select_rownames(.summary_betas, 'beta'))/p), each=p))
  # pval_res_df <- cbind.data.frame(type_beta=(names_covar),
  #                                 # x=-Inf, y=Inf, pval=pval_res)
  #                                 x=1, y=0, pval=pval_res)
  if(annotate_pval){
    .summary_betas$type_beta = paste0(.summary_betas$type_beta, '\npval=',
                                      signif(pval_res[match(.summary_betas$type_beta, names_covar)], 3))
  }
  

  if(!interchange_facet_x){
    plt <- ggplot(.summary_betas, aes(x=LogR, y=`Estimate`))
    plt <- plt +
      geom_point()+
      geom_errorbar(aes(ymin=`Estimate`-`Std. Error`, ymax=`Estimate`+`Std. Error`), width=.1)+
      facet_wrap(.~factor(type_beta, levels=unique(type_beta)), scales = "free")+theme_bw()+
      geom_hline(yintercept = 0, lty='dashed', col='blue')
  }else{
    plt <- ggplot(.summary_betas, aes(x=factor(type_beta, levels=unique(type_beta)), y=`Estimate`))
    plt <- plt +
      geom_point()+
      geom_errorbar(aes(ymin=`Estimate`-`Std. Error`, ymax=`Estimate`+`Std. Error`), width=.1)+
      facet_wrap(.~LogR, scales = "free")+theme_bw()+
      geom_hline(yintercept = 0, lty='dashed', col='blue')
  }
  
  plt+ theme(axis.text.x = element_text(angle = angle_arg, vjust = 1, hjust=1))


  
  # if(annotate_pval){
  #   plt <- plt +
  #     geom_text(data=pval_res_df, aes(x=x, y=y, label=paste0('p. value=', signif(pval, 3))),
  #               col='red'
  #               # nudge_x = 3, nudge_y = 2.2
  #               )
  # }
...
plot_betas <- function(TMB_obj, names_cats=NULL, rotate_axis=T, theme_bw=T, remove_SBS=T, only_slope=F, return_df=F, plot=T,
                       ...
                       size_title=NULL, size_logR=NULL, xlab=NULL, betas_are_only_slope=F, p=2, names_covariates=NULL){
  if(p != 2){
    plot_betas_generalised(TMB_obj, return_plot, return_ggplot, p, names_covariates)
  }else{
